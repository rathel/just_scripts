...
sudo nala upgrade -y
sudo nala install xautolock wireplumber lxpolkit unzip alacritty vim git tmux build-essential libxrandr-dev libxft-dev libxinerama-dev lightdm lightdm-gtk-greeter dmenu curl distrobox bluetooth bluez network-manager rsync picom -y
...
if ! cat $HOME/.xsessionrc | grep .nix-profile/share; then
	echo 'export XDG_DATA_DIRS="$HOME/.nix-profile/share:$XDG_DATA_DIRS:/usr/share"' >>$HOME/.xsessionrc
fi
if ! cat $HOME/.xsessionrc | grep .nix-profile/bin; then
...
if ! cat $HOME/.config/nixpkgs/config.nix | grep Unfree; then
